# search.jl — include from uci.jl after `using Chess` and global declarations
...
function quiescence(b::Board, α::Int, β::Int, ply::Int, node_count::Ref{Int})::Int
    node_count[] += 1
    search_stopped[] && return 0

    # Explicit game-over check before stand-pat
    if ischeckmate(b)
    ...
    # Captures only — collect and sort by MVV-LVA
    captures = filter(m -> moveiscapture(b, m), moves(b))
    isempty(captures) && return α

    sort!(captures, by = m -> begin
        vt       = ptype(pieceon(b, to(m))).val
        victim   = vt > 6 ? 1 : vt  # en passant: captured pawn not on to(m), treat as pawn
        attacker = ptype(pieceon(b, from(m))).val
        _QS_PIECE_VAL[victim] - _QS_PIECE_VAL[attacker]
    end, rev = true)
    ...
    for m in captures
        ...
        u  = domove!(b, m)
        sc = -quiescence(b, -β, -α, ply + 1, node_count)
        undomove!(b, u)
...
function negamax(b::Board, depth::Int, α::Int, β::Int, ply::Int, pv::Vector{Move}, node_count::Ref{Int})::Int
    node_count[] += 1
    search_stopped[] && return 0
    ...
    if depth == 0
        empty!(pv)
        return quiescence(b, α, β, ply, node_count)
    end

    # Move ordering: TT best move first
    if tt_best != Move(0)
        bi = findfirst(==(tt_best), ml)
        if bi !== nothing && bi > 1
            ml[1], ml[bi] = ml[bi], ml[1]
        end
    end
    ...
    for m in ml
        empty!(child_pv)
        u  = domove!(b, m)
        sc = -negamax(b, depth - 1, -β, -α, ply + 1, child_pv, node_count)
        undomove!(b, u)
...
function search(b::Board, max_depth::Int, time_limit::Int)::Move
    start_ns  = time_ns()
    deadline  = time_limit >= typemax(Int) >> 20 ? typemax(UInt64) :
                start_ns + UInt64(time_limit) * 1_000_000
    ...
    for depth in 1:max_depth
        ...
        for m in ml
            ...
            empty!(child_pv)
            u  = domove!(b, m)
            sc = -negamax(b, depth - 1, -INF, -α, 1, child_pv, node_count)
            undomove!(b, u)
